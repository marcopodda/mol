...
class Wrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        decoder_bce_loss = F.binary_cross_entropy_with_logits(mlp_outputs, mlp_targets)
        cos_sim = F.cosine_similarity(decoder_bag_of_frags, encoder_bag_of_frags).mean(dim=0)

        total_loss = decoder_ce_loss + decoder_bce_loss
        result = pl.TrainResult(minimize=total_loss)
        result.log('DL', decoder_ce_loss, prog_bar=True)
        result.log('bce', decoder_bce_loss, prog_bar=True)
